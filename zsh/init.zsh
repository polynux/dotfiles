...
function zprezto-update {
  (
    ...
    if [[ "$orig_branch" == "master" ]]; then
      ...
      printf "zprezto install at '%s' is not on the master branch " "${ZCONFIG}"
...
function pmodload {
  ...
  pmodule_dirs=("$ZCONFIG/modules" "$ZCONFIG/contrib" "$user_pmodule_dirs[@]")
...
if [[ -s "$ZCONFIG/zpreztorc" ]]; then
  source "$ZCONFIG/zpreztorc"
fi
